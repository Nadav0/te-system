...
function StepIndicator({ current }: { current: 1 | 2 | 3 }) {
 ...
 : active
 ? 'bg-brand-600 border-brand-600 text-white'
 : 'bg-surface-1 border-edge text-ink-3'
 }`}
 >
 ...
 active ? 'text-brand-600' : done ? 'text-ink-3' : 'text-ink-3'
 }`}
 >
 {s.label}
 </span>
 ...
 current > s.n ? 'bg-brand-600' : 'bg-surface-hover'
 }`}
 />
 )}
 </div>
...
function ReportPrintView({
 ...
 return (
 <div className="hidden print:block p-8 font-sans text-ink">
 <div className="mb-6 border-b border-edge-hi pb-4">
 ...
 <thead>
 <tr className="border-b-2 border-edge-hi">
 ...
 <tbody>
 {travel.map((t) => (
 <tr key={t.id} className="border-b border-edge">
...
export default function ReportsPage() {
 ...
 : 'bg-surface-1 text-ink-2 border-edge hover:border-edge-hi'
 }`}
 >
 ...
 <p className="text-ink-3 font-semibold">No records match your filters</p>
 <p className="text-sm text-ink-3 mt-1">Go back and adjust your filters.</p>
 </div>
 ) : (
 ...
 <h3 className="font-semibold text-ink">Expense Reports</h3>
 <span className="text-xs text-ink-3">{filteredExpenses.length} records</span>
 </div>
 <table className="w-full text-sm">
 <thead className="bg-surface-0">
 <tr>
 <th className="text-left px-5 py-2.5 text-xs font-semibold text-ink-3 uppercase tracking-wider">Title</th>
 <th className="text-left px-5 py-2.5 text-xs font-semibold text-ink-3 uppercase tracking-wider">Employee</th>
 <th className="text-left px-5 py-2.5 text-xs font-semibold text-ink-3 uppercase tracking-wider">Status</th>
 <th className="text-right px-5 py-2.5 text-xs font-semibold text-ink-3 uppercase tracking-wider">Amount</th>
 <th className="text-left px-5 py-2.5 text-xs font-semibold text-ink-3 uppercase tracking-wider">Created</th>
 </tr>
 </thead>
 ...
 {r.employee?.department && (
 <p className="text-xs text-ink-3">{r.employee.department}</p>
 )}
 </td>
 ...
 <td className="px-5 py-3 text-ink-3">{date(r.created_at)}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <div className="bg-surface-1 border border-edge rounded-xl overflow-hidden">
 <div className="px-5 py-3 border-b border-edge flex items-center justify-between">
 <h3 className="font-semibold text-ink">Travel Requests</h3>
 <span className="text-xs text-ink-3">{filteredTravel.length} records</span>
 </div>
 <table className="w-full text-sm">
 <thead className="bg-surface-0">
 <tr>
 <th className="text-left px-5 py-2.5 text-xs font-semibold text-ink-3 uppercase tracking-wider">Destination</th>
 <th className="text-left px-5 py-2.5 text-xs font-semibold text-ink-3 uppercase tracking-wider">Employee</th>
 <th className="text-left px-5 py-2.5 text-xs font-semibold text-ink-3 uppercase tracking-wider">Dates</th>
 <th className="text-left px-5 py-2.5 text-xs font-semibold text-ink-3 uppercase tracking-wider">Status</th>
 <th className="text-right px-5 py-2.5 text-xs font-semibold text-ink-3 uppercase tracking-wider">Budget</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-edge">
 {filteredTravel.map((t) => (
 <tr key={t.id} className="hover:bg-surface-hover">
 <td className="px-5 py-3 font-medium text-ink">{t.destination}</td>
 <td className="px-5 py-3 text-ink-2">
 <p>{t.employee?.full_name ?? '—'}</p>
 {t.employee?.department && (
 <p className="text-xs text-ink-3">{t.employee.department}</p>
 )}
 </td>
 ...
 <Icon
 size={28}
 className={`mb-3 ${exportFormat === format ? 'text-brand-600' : 'text-ink-3'}`}
 />
 <p className={`font-semibold text-sm mb-1 ${exportFormat === format ? 'text-brand-700' : 'text-ink'}`}>
 {title}
 </p>
