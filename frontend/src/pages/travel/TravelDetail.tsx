...
export default function TravelDetail() {
 ...
 if (!tr) return <div className="p-8 text-ink-3">Travel request not found</div>
 ...
 <div>
 <p className="text-xs text-ink-3">Destination</p>
 <p className="font-semibold text-ink">{tr.destination}</p>
 </div>
 </div>
 <div className="card flex items-center gap-3 py-4">
 <DollarSign className="text-green-500 flex-shrink-0" size={20} />
 <div>
 <p className="text-xs text-ink-3">Estimated Budget</p>
 <p className="font-semibold text-ink">{currency(tr.estimated_budget)}</p>
 </div>
 </div>
 <div className="card flex items-center gap-3 py-4">
 <Calendar className="text-indigo-500 flex-shrink-0" size={20} />
 <div>
 <p className="text-xs text-ink-3">Travel Dates</p>
 <p className="font-semibold text-ink">{date(tr.departure_date)} → {date(tr.return_date)}</p>
 </div>
 </div>
 <div className="card flex items-center gap-3 py-4">
 ...
 <h3 className="font-semibold text-ink">Business Purpose</h3>
 </div>
 <p className="text-ink-2 text-sm leading-relaxed">{tr.purpose}</p>
 </div>

 {tr.review_note && (
 <div className={`mb-6 p-4 rounded-lg border ${tr.status === 'approved' ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
 <p className="text-sm font-medium text-ink-2 mb-1">Reviewer note ({tr.reviewer?.full_name}):</p>
 ...
 <div className="card mb-6">
 <h3 className="font-semibold text-ink mb-3">Linked Expense Reports</h3>
 <div className="space-y-2">
 {tr.expense_reports.map((r: any) => (
 <Link key={r.id} to={`/expenses/${r.id}`}
 className="flex items-center justify-between p-2 rounded-lg hover:bg-surface-hover border border-edge">
 <span className="text-sm text-ink-2">{r.title}</span>
 ...
 {canReview && (
 <div className="card">
 <h3 className="font-semibold text-ink mb-4">Review This Request</h3>
 <textarea
 value={reviewNote}
